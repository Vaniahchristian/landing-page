...
function TreeSVG({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 48 60" fill="none" className={className}>
      <rect x="21" y="45" width="6" height="13" rx="1.5" fill="currentColor" opacity="0.45" />
      <polygon points="24,22 7,45 41,45" fill="currentColor" opacity="0.65" />
      <polygon points="24,12 10,34 38,34" fill="currentColor" opacity="0.8" />
...
export default function Sustainability() {
  ...
  return (
    <section id="sustainability" className="relative bg-[#1B3A2D] py-32 px-6 overflow-hidden">
      ...
      />
      {/* Flanking trees */}
      <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/3 opacity-[0.06] pointer-events-none">
        <TreeSVG className="w-64 h-80 text-[#F5F0E8]" />
      </div>
      <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/3 opacity-[0.06] pointer-events-none scale-x-[-1]">
        <TreeSVG className="w-64 h-80 text-[#F5F0E8]" />
      </div>
      {/* Top fade from categories (dark) */}
      ...
      <div ref={ref} className="relative z-10 max-w-2xl mx-auto text-center">
        ...
        >
          <div className="relative" style={{ animation: 'floatTree 5s ease-in-out infinite' }}>
            <TreeSVG className="w-12 h-[3.5rem] text-[#8B6914]" />
            <div className="absolute inset-0 blur-2xl bg-[#8B6914]/20 -z-10 scale-[1.6]" />
          </div>
        </motion.div>
        ...
        <motion.p
          ...
          className="text-lg text-[#F5F0E8]/50 mb-12"
        >
        ...
        <motion.div
          ...
          className="inline-flex items-center gap-4 px-7 py-4 rounded-full border border-[#F5F0E8]/12 bg-[#F5F0E8]/[0.05]"
        >
          <span className="text-2xl font-bold text-[#8B6914] tabular-nums">
            <KgCounter />kg+
          </span>
          <span className="text-sm text-[#F5F0E8]/45">carbon offset and counting</span>
        </motion.div>
      </div>
    </section>
  )
